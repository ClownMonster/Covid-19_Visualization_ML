...
df_world_recovered = pd.read_csv('../Data_contents/time_series_covid19_recovered_global.csv')
df_world_recovered.drop( ['Province/State', 'Lat', 'Long'], axis = 1, inplace = True )

def print_header():
    '''' print(df_India.columns.values) this is to print the headers list this can
    done with  print(list(df_India)) '''
    print(list(df_world_recovered))
...
def graphVisualizationRecovered(date):
    ...
    '''
    global df_world_recovered
    df_world_recovered.sort_values(by = date, ascending = False, inplace = True)

    fig = px.bar(df_world_recovered, x='Country/Region',
